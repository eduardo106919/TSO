...
static int xmp_getattr(const char* path, struct stat* stbuf,
    ...
    Filemeta meta;
    if (index_get(p_ctx->index, path, &meta) != -1) {
...
static int xmp_read(const char* path, char* buf, size_t size, off_t offset,
    ...
    char compressed[meta.real_size];

    // read the compressed data
    res = pread(fd, compressed, meta.real_size, 0);
    if (res == -1) return -errno;

    unsigned int temp_size = meta.logical_size;
    char data[meta.logical_size];

    // decompress the data
    decompress(compressed, meta.real_size, data, &temp_size);

    // copy the desired data portion
...
static int xmp_release(const char* path, struct fuse_file_info* fi) {
    ...
    if ((fi->flags & O_WRONLY) || (fi->flags & O_RDWR)) {
        ...
        // read file content
        res = pread(fi->fh, buf, st.st_size, 0);
        if (res == -1) return -errno;

        unsigned int new_size = st.st_size + 1024;
        char compressed[new_size];

        // compress the data
        res = compress(buf, st.st_size, compressed, &new_size);
        if (res != 0) return -errno;

        // write the compressed data
        res = pwrite(fi->fh, compressed, new_size, 0);
        if (res == -1) return -errno;

        res = close(fi->fh);

        Filemeta meta = {.logical_size = st.st_size, .real_size = new_size};
        // store metadata
        index_add(p_ctx->index, path, meta);
    }
